    ...
    def __init__(self,
        ...
        super().__init__(
            nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, bias=bias,
                      dilation=dilation, stride=stride, groups=groups, padding=((stride - 1) + dilation * (kernel_size - 1)) // 2),
        )
...
class GlobalLocalAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.merge_conv = ConvBN(dim, dim, kernel_size=7, groups=dim)
        self.merge_1x1 = Conv(dim, dim, kernel_size=1)
    ...
    def forward(self, x, mask=None):
        """
        Args:
            x: input features with shape of (num_windows*B, N -> Wh*Ww, C -> dim)
        ...
        if mask is not None:
            H = W = int((nW * self.window_size[0] * self.window_size[1]) ** 0.5)
            # view: num_windows*B, window_size, window_size, C
            # window_reverse: B, H, W, C
            # permute: B, C, H, W
            local = local.view(B_, self.window_size[0], self.window_size[1] , C)
            local = window_reverse(local, self.window_size[0], H, W).permute(0, 3, 1, 2).contiguous()
            # local: B, C, H, W
            local = self.local2(local) + self.local1(local)

            x = window_reverse(x, self.window_size[0], H, W).permute(0, 3, 1, 2).contiguous()
            x = x + local
            x = self.merge_conv(x)
            x = self.merge_1x1(x)
            # x: B, H, W, C
            ...
            x = window_partition(x, self.window_size[0]).view(B_, N, C)
